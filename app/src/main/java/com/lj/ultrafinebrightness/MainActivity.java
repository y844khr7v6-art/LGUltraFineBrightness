...
public class MainActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle b){
        ...
        findViewById(R.id.b100).setOnClickListener(v->setBrightnessPercent(100,true));
        slider.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener(){
            ...
            public void onStopTrackingTouch(SeekBar s){handler.removeCallbacks(pendingWrite);setBrightnessPercent(s.getProgress(),false);}
        });
    ...
    private void openDevice(){
        ...
        if(connection==null){setStatus("Could not open UltraFine USB control device");return;}
        ...
        handler.postDelayed(this::probeAllReadOnly,250);
    }
    ...
    private void probeAllReadOnly(){
        ...
        StringBuilder out=new StringBuilder("READ-ONLY HID PROBE\n043e:9a63 LG UltraFine Display Controls\n\n");
        out.append(describeInterface(i2cInterface,"Interface 0 · HID I2C"));
        out.append(describeInterface(brightnessInterface,"Interface 1 · HID BRIGHTNESS"));
        out.append(describeInterface(alsInterface,"Interface 2 · HID ALS"));
        probeOutput.setText(out.toString());
    }

    private void probeInterface(int id,String name){
        if(connection==null){setStatus("Not connected");return;}
        UsbInterface intf=id==IF_I2C?i2cInterface:id==IF_ALS?alsInterface:brightnessInterface;
        probeOutput.setText("READ-ONLY PROBE\n\n"+describeInterface(intf,"Interface "+id+" · "+name));
    }

    private String describeInterface(UsbInterface intf,String label){
        StringBuilder s=new StringBuilder();s.append(label).append("\n");
        if(intf==null){s.append("  unavailable / not claimed\n\n");return s.toString();}
        s.append("  class=").append(hex2(intf.getInterfaceClass())).append(" subclass=").append(hex2(intf.getInterfaceSubclass())).append(" protocol=").append(hex2(intf.getInterfaceProtocol())).append("\n");
        s.append("  endpoints=").append(intf.getEndpointCount()).append("\n");
        for(int e=0;e<intf.getEndpointCount();e++){
            UsbEndpoint ep=intf.getEndpoint(e);
            s.append("    ep").append(e).append(" addr=").append(hex2(ep.getAddress())).append(" dir=").append(ep.getDirection()==UsbConstants.USB_DIR_IN?"IN":"OUT").append(" type=").append(endpointType(ep.getType())).append(" maxPacket=").append(ep.getMaxPacketSize()).append(" interval=").append(ep.getInterval()).append("\n");
        }
        byte[] desc=new byte[2048];
        int dn=connection.controlTransfer(0x81,0x06,0x2200,intf.getId(),desc,desc.length,1000);
        s.append("  HID report descriptor: ").append(dn>=0?dn+" bytes":"FAILED "+dn).append("\n");
        if(dn>0)s.append("    ").append(toHex(desc,Math.min(dn,256))).append(dn>256?" …":"").append("\n");
        s.append("  GET_REPORT reads (no writes):\n");
        int found=0;
        for(int reportType:new int[]{1,3}){
            for(int reportId=0;reportId<16;reportId++){
                byte[] b=new byte[64];
                int n=connection.controlTransfer(0xA1,0x01,(reportType<<8)|reportId,intf.getId(),b,b.length,120);
